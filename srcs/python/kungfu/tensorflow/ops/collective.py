...
def group_hierarchical_nccl_all_reduce(ts):
    ...
    if len(names) == 1:
        ...
        schedule_local_reduce = 'local_reduce'
        schedule_local_broadcast = 'local_broadcast'
        ...
        with tf.control_dependencies([
                _start_nccl_scheduler(t_names,
                                      scheduler=schedule_local_reduce,
                                      scope='local'),
        ]):
            xs = map_maybe(local_reduce, ts)
            ys = map_maybe(cross_all_reduce, xs)
            y_names = map_maybe(lambda y: y.name, ys)
            with tf.control_dependencies(xs + [
                    _start_nccl_scheduler(y_names,
                                          scheduler=schedule_local_broadcast,
                                          scope='local'),
            ]):
                zs = map_maybe(local_broadcast, ys)
                return zs
